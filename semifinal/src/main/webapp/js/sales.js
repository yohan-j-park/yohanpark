 ...
 */
 ...
 if(frmInput != null){
	frmInput.btnCancel.onclick = function(){
		frmInput.action = 'sales.do?job=select';
		frmInput.submit();
	}
	...
	frmInput.btnSave.onclick = function(){
		frmInput.action = 'sales_result.jsp?job=insertR';
		frmInput.submit();
	}
}
 ...
 if(frmSearch != null){
	frmSearch.btnInsert.addEventListener('click', function(){		
		frmSearch.action = 'sales.do?job=insert';
		frmSearch.submit();
	});
	
	frmSearch.btnSelect.onclick = function(){		
		frmSearch.action = 'sales.do?job=select';		
	...
	//통계자료 사용하기
	/*
	frmStat.btnStat.onclick = function(){
		frmSearch.action = 'sales.do?job=stat';
		frmSearch.submit();
	}
	*/
}
...
function view(pr_code){
	frmSearch.action = 'sales.do?job=update&pr_code='+pr_code;
	frmSearch.submit();
}
...
if(btnSelect != null){
	btnSelect.onclick = function(){
		...
		frm.action = 'sales.do?job=select';
		frm.submit();
	}
}
...
if(btnModify != null){
	btnModify.onclick = function(){
		var frm = document.frm_sales;
		frm.action = 'sales_result.jsp?job=updateR';	//R:result
		frm.submit();
	}
}
...
if(btnDelete != null){
	btnDelete.onclick = function(){
		var frm = document.frm_sales;
		frm.action = 'sales_result.jsp?job=deleteR';
		frm.submit();
	}
}
