		...
		<video src="video/computer_screen.mp4" autoplay loop muted plays-inline class="back-video">
				...
				<div id=table_wrapper>
					...
					<form id = "table" name = 'frm_search' method = 'post'>
						<div id = salesSearch>
							<span>생산일자</span>
		            		<input type = "date" name='findDate' value ="${pageVo.findDate }" min = "2023-01-01"/>
							<span>판매일자</span>
		            		<input type = "date" name='findUpdate_time' value ="${pageVo.findUpdate_time }" min = "2023-01-01"/>
		            		<span>제품이름</span>
		            		<select name='findPrname'>
		             			<option value=""></option>
		            		...
		            		<select name='findStatus'>
		           			...
		           			<span>제품코드</span>
		            		<input type="text" name="findPrcode" value="${pageVo.findPrcode }" placeholder="제품 코드를 입력해 주세요"/>
		           		    <input type = "button" class="btnSelect" value = "조회" name = 'btnSelect'/>
		            		<br/><br/>
							<input type = "button" class="btnInsert" value = "입력" name = 'btnInsert' />

							<input type = 'hidden' name = 'nowPage' value = "${pageVo.nowPage }"/>
							<input type = "button" class="btnStat" value = "통계" name = 'btnStat' />
					...
					<div id = "r1">
						...
						<span class = "c5">update_time</span>
						<span class = "c6">pr_cost</span>
						<span class = "c7">real_price</span>
						<span class = "c8">vat</span>
						<span class = "c9">supply_price</span>
					</div>
			...
			<div class = "pageNumber"><!-- page 이동 버튼 -->
				<c:if test="${pageVo.startPage>1 }">
					...
					<button style=" cursor: pointer;" class='btn' onclick = 'movePage(${pageVo.startPage-1})'><</button>
				</c:if>
				
				
				<c:forEach var='i' begin='${pageVo.startPage }' end='${pageVo.endPage }'>
					<button style="cursor: pointer;" class='btn' onclick = 'movePage(${i})'>${i }</button>
				</c:forEach>
				
				<c:if test="${pageVo.totPage gt pageVo.endPage }">
					<button style="cursor: pointer;" class='btn' onclick = 'movePage(${pageVo.endPage+1})'>></button>
					<button style="cursor: pointer;" class='btn' onclick = 'movePage(${pageVo.totPage})'>>></button>
				</c:if>
			</div>						
		</div>
	</div>
